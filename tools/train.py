...
def parse_args():
    ...
    parser.add_argument('-i', '--pre-trained-id', dest='pt_id',
                        help='the pre-trained network id that you want to load for testing', type=str, required=False)
    parser.add_argument('-p', '--pre-trained-epoch', dest='pt_epoch',
                        help='the epoch at which a snapshot for the id is taken', type=str, required=False)
...
def set_positional_cfg(args_in):
    ...
    for n, a in args_in.__dict__.items():
        if a is not None and n not in ['cfg_file', 'set_cfg', 'pt_id', 'pt_epoch']:
            args_list += [n, a]
    return args_list
...
if __name__ == '__main__':
    ...
    if args.pt_id:
        assert args.pt_epoch, 'you must set the epoch for the pre-trained model'
        cfg.PT_PATH = os.path.join(cfg.EXPERIMENT_DIR,
                                   'snapshot_{}_{}'.format(cfg.DATASET_NAME, cfg.NET_ARCH),
                                   args.pt_id,
                                   '{:03}.pt'.format(int(args.pt_epoch)))
